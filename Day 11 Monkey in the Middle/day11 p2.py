...
def main():
    ...
    cycle_length = 1
    for name in troop: cycle_length *= troop[name][TEST]
    print(cycle_length)
    for i in tqdm(range(10000)): simulate(cycle_length) #tgdm wraps the loop in a progress bar
...
def simulate(cycle_length):
    for name, stat in troop.items():
        for item in stat[ITEMS]:
            ...
            item = eval(stat[MULTIPLIER]) #eval takes a string and outputs python code
