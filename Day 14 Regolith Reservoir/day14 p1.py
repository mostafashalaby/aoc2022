...
def encompass_with_void():
    global POURING_FROM
    for i, row in enumerate(mine):
        temp = ["X"] + row + ["X"]
        mine[i] = temp
    temp = ["X" for _ in range(len(mine[0]))]
    mine.append(temp)
    POURING_FROM = (POURING_FROM[0] + 1, POURING_FROM[1])


def is_void(char):
    return char == RESOURCES[VOID]
...
def send_sand(current_point):
    ...
    if is_void(char):
        return False
    elif mine[cy + 1][cx] in [RESOURCES[AIR], RESOURCES[VOID]]:  # down
        return send_sand((cx, cy + 1))
    elif mine[cy + 1][cx - 1] in [RESOURCES[AIR], RESOURCES[VOID]]:  # diagonal left
        return send_sand((cx - 1, cy + 1))
    elif mine[cy + 1][cx + 1] in [RESOURCES[AIR], RESOURCES[VOID]]:  # diagonal right
        return send_sand((cx + 1, cy + 1))
    else:
        return cx, cy
